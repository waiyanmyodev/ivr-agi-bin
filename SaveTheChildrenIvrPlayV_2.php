...
for ($i = 1; $i <= 3; $i++) {
    ...
    if ($chosenLang === "") {
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            $childAgeRange = $agi->get_data('ngo_ivr/mm/mm_topic_intro', 7000, 1);

            $agi->stream_file('ngo_ivr/mm/mm_key_message', 115000);
            ...
            for ($i = 1; $i <= 3; $i++) {
                ...
                if ($quizPress1Result['result'] == 2) {
                    ...
                    $agi->stream_file('ngo_ivr/mm/mm_quiz_correct', 8000);
                    $q1_answer = 2;
                    $conn->close();
                    break;
                } elseif ($quizPress1Result['result'] == 1) {
                    // incorrect answer for press 1
                    $agi->stream_file('ngo_ivr/mm/mm_quiz_incorrect', 9000);
                    $q1_answer = 1;
                    break;
                } else {
                    // play invalid input
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            // mm intro and ask child age,  child age range (1 for 0-3, 2 for > 3)
            $childAgeRange = $agi->get_data('ngo_ivr/pok/poe_intro', 11000, 1);



            // key message if pressed 1
            $agi->stream_file('ngo_ivr/pok/poe_key_message', 146000);

            for ($i = 1; $i <= 3; $i++) {

                // quiz if press 1
                $quizPress1Result = $agi->get_data('ngo_ivr/pok/poe_quiz', 48000, 1);

                if ($quizPress1Result['result'] == 2) {
                    // correct anaswer for press 2
                    $agi->stream_file('ngo_ivr/pok/poe_quiz_correct', 17000);
                    $q1_answer = 2;
                    ...
                    // incorrect answer for press 1
                    $agi->stream_file('ngo_ivr/pok/poe_quiz_incorrect', 19000);
                    $q1_answer = 1;
        ...
        // outro
        $agi->stream_file('ngo_ivr/pok/poe_outtro', 24000);
        // break loop
        break;
    } elseif ($chosenLang == 3) {
        for ($i = 1; $i <= 3; $i++) {
            ...
            // mm intro and ask child age,  child age range (1 for 0-3, 2 for > 3)
            $childAgeRange = $agi->get_data('ngo_ivr/skk/sg_topic_intro', 18000, 1);



            // key message if pressed 1
            $agi->stream_file('ngo_ivr/skk/sg_key_message', 223000);

            for ($i = 1; $i <= 3; $i++) {

                // quiz if press 1
                $quizPress1Result = $agi->get_data('ngo_ivr/skk/sg_quiz', 60000, 1);

                if ($quizPress1Result['result'] == 2) {
                    // correct anaswer for press 2
                    $agi->stream_file('ngo_ivr/skk/sg_quiz_correct', 17000);
                    $q1_answer = 2;
                    ...
                    // incorrect answer for press 1
                    $agi->stream_file('ngo_ivr/skk/sg_quiz_incorrect', 20000);
                    $q1_answer = 1;
        ...
        // outro
        $agi->stream_file('ngo_ivr/skk/sg_outro', 32000);
